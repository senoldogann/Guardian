...
```bash
# Check the shared source first, then provider bridges and legacy fallbacks
unset B
for candidate in \
  .maestro/skills/gstack/browse/dist/browse \
  .agent/skills/gstack/browse/dist/browse \
  .claude/skills/gstack/browse/dist/browse \
  .agents/skills/gstack/browse/dist/browse \
  .opencode/skills/gstack/browse/dist/browse \
  ~/.claude/skills/gstack/browse/dist/browse
do
...
Set `B` to whichever path is READY and use it for all commands. Prefer `.maestro/skills/gstack/...` when available.

If `NEEDS_SETUP`:
1. Tell the user: "gstack browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait for their response.
2. If they approve, determine the skill directory (preferred `.maestro/skills/gstack`, then any provider bridge such as `.agent/skills/gstack`, `.claude/skills/gstack`, `.agents/skills/gstack`, `.opencode/skills/gstack`, or the legacy user-level `~/.claude/skills/gstack`) and run:
```bash
cd <SKILL_DIR> && ./setup
```
...
- Use the compiled binary via Bash through the resolved `$B` path.
...
```bash
B=.maestro/skills/gstack/browse/dist/browse
