...
async function loadLatestReleaseNotes(locale: Locale): Promise<{
  highlights: string[];
  isTr: boolean;
  ...
  try {
    ...
    return {
      highlights: pickHighlights(latest),
      isTr,
      publishedLabel: formatReleaseDate(latest.publishedAt, isTr ? "tr-TR" : "en-US"),
      releaseTitle: isTr ? `${latest.tag} sürümünde neler yeni?` : `What's New in ${latest.tag}`,
    };
  } catch {
    return null;
  }
...
export async function LatestReleaseNotes({ locale }: { locale: Locale }) {
  const latestReleaseNotes = await loadLatestReleaseNotes(locale);
  if (!latestReleaseNotes) {
    return null;
  }

  const { highlights, isTr, publishedLabel, releaseTitle } = latestReleaseNotes;
