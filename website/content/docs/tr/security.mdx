...
## Governance güvenlik garantileri

Guardian'ın güvenlik modeli yalnızca static scan'e değil, release kararına hizalanır:

- Policy kuralları repo içinde versiyonlu tutulur (`guardian.policy.yaml`) ve desktop + CLI + CI tarafında tutarlı değerlendirilir.
- İnsan onayı ve override kararları `.guardian/release_decisions.jsonl` ile denetlenebilir şekilde kaydedilir.
- Local-first çalışma modeli sayesinde kaynak bağlam, harici provider'ları özellikle açmadıkça workspace içinde kalır.
- Release gate çıktıları (`PASS`, `PASS_WITH_WARNING`, `BLOCK_UNTIL_APPROVED`, `OVERRIDDEN`) yayın öncesinde açık sorumluluk zinciri kurar.
