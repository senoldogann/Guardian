...
## v1.2.0 Stabilization Notes

v1.2.0 is a stabilization release. Key fixes include:

- Monitor hydration from `.guardian/critiques.json` on launch and after reconnects.
- Baseline view defaults stay on **All** unless you explicitly choose **New** or **Resolved**.
- Embedding auto mode skips OpenAI when no key exists and uses Ollama → local hash fallback.
- Roadmap entries for the stabilization phases were recorded for traceability.
...
├── Guardian_1.2.2_aarch64.app.tar.gz   # macOS updater artifact
├── Guardian_1.2.2_aarch64.dmg          # macOS installer
└── ...
```
...
```json
{
  ...
  "platforms": {
    "darwin-aarch64": {
      ...
      "url": "https://github.com/<org>/<repo>/releases/download/v1.2.2/Guardian_1.2.2_aarch64.app.tar.gz"
    }
  }
}
```
...
```json
{
  ...
  "downloads": {
    "darwin": {
      "url": "https://internal.company.com/guardian/Guardian-1.2.2.dmg"
    }
  }
}
```
