...
v1.2.0 is a stabilization release focused on predictable monitoring and resilient embeddings.

- **Monitor hydration**: Existing findings are loaded from `.guardian/critiques.json` on launch and merged with live updates.
- **Default filters stay All**: Baseline view no longer auto-switches to New; select New/Resolved manually when needed.
- **Embedding auto routing**: Auto mode uses OpenAI only when a valid key exists; otherwise it goes directly to Ollama and falls back to local hash.
- **Cleanup + verification**: Removed an unused fix-proposal helper and expanded tests in desktop + Tauri; full regression runs were recorded across desktop, Tauri, CLI, website, and the verify_all script.
