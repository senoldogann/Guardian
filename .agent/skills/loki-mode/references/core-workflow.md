...
**PURPOSE:** Prevent context bloat by keeping CONTINUITY.md lean and relevant.

### Section Limits

| Section | Max Items | Pruning Action |
|---------|-----------|----------------|
| Just Completed | 5 | Archive oldest to episodic memory |
| Key Decisions | 10 | Archive oldest to semantic memory |
| Mistakes & Learnings | 10 | Consolidate to semantic patterns |
| Next Actions | 5 | Move completed to Just Completed |
| Active Blockers | 3 | Escalate or resolve stale blockers |

### Auto-Prune Protocol

**When:** At the end of every major phase transition or every 10 iterations.

```python
def prune_continuity():
    """Keep CONTINUITY.md under 200 lines."""
    
    # 1. Just Completed: Keep only last 5
    if len(just_completed) > 5:
        archive_to_episodic(just_completed[5:])
        just_completed = just_completed[:5]
    
    # 2. Key Decisions: Keep only last 10
    if len(key_decisions) > 10:
        archive_to_semantic(key_decisions[10:])
        key_decisions = key_decisions[:10]
    
    # 3. Mistakes & Learnings: Consolidate patterns
    if len(mistakes) > 10:
        consolidated = consolidate_to_patterns(mistakes[10:])
        save_to_semantic(consolidated)
        mistakes = mistakes[:10]
    
    # 4. Clear resolved blockers
    blockers = [b for b in blockers if not b.resolved]
```

### Manual Pruning Trigger

Run `scripts/prune_memory.py` for comprehensive cleanup:

```bash
# Full prune (episodic, handoffs, queue, CONTINUITY.md)
python scripts/prune_memory.py

# Dry run (preview what would be pruned)
python scripts/prune_memory.py --dry-run
```
