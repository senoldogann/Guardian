...
**RULE:** Receiving agents MUST acknowledge handoffs before proceeding.

```python
def acknowledge_handoff(handoff_id: str, agent_id: str):
    """
    Mark handoff as acknowledged by receiving agent.
    Prevents lost context and enables handoff tracking.
    """
    handoff = load_handoff(handoff_id)
    
    if handoff.to_agent != agent_id:
        raise HandoffError(f"Agent {agent_id} not authorized for handoff {handoff_id}")
    
    handoff.acknowledged_at = datetime.now().isoformat()
    handoff.acknowledgment = {
        "agent_id": agent_id,
        "timestamp": handoff.acknowledged_at,
    ...
    # Prune acknowledged handoffs older than 24h (via prune_memory.py)
    # This prevents handoff directory bloat

def before_task_start(agent_id: str, task_id: str):
    """
    Check for pending handoffs before starting any task.
    """
    pending = find_pending_handoffs(to_agent=agent_id)
    
    for handoff in pending:
        if handoff.acknowledged_at is None:
            # Read context from handoff
            inject_context(handoff.context)
            # Acknowledge receipt
            acknowledge_handoff(handoff.id, agent_id)
```

### Handoff Lifecycle

```
1. CREATED: from_agent writes handoff to .loki/memory/handoffs/
2. PENDING: to_agent has not acknowledged (acknowledged_at = null)
3. ACKNOWLEDGED: to_agent sets acknowledged_at timestamp
4. ARCHIVED: Pruned after 24h by prune_memory.py
```
```
